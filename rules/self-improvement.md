...
| Agent dispatch must be scoped to ONE file + ONE failure class. Never dispatch "fix N tests" or "fix all failures" — broad agents spend 30-40 tool calls reading before getting cut off with zero output. Always run `pytest -x` first, identify the first failure's root cause, then dispatch one agent per root cause. | 4 backend-expert agents dispatched to "fix 70 tests" each got cut off mid-report — Sprint 31 2026-07-28 |
| Always arm a Monitor immediately after launching any background process (pipeline, self-improvement loop, build). Filter must cover both success and failure paths. When Monitor fires, handle the result automatically without waiting for user prompt. Never use `sleep` polling as a substitute. | User had to repeatedly ask "check the post when it finishes" because Monitor was not auto-armed after pipeline launch — medium-agent-factory Sprint 36-37 2026-07-29 |
| For iterative prompt/pipeline sprints (LLM output quality loops), auto-proceed with the recommended fix and launch without asking the user to approve each iteration. User always picks the recommendation — pausing wastes context and breaks flow. | User explicitly confirmed: "auto approve your thoughts and questions, because you always choose the recommended" — medium-agent-factory Sprint 37 2026-07-29 |
| Never spawn more than one Python linting/formatting process at a time. `black --check` on Python 3.14 spawns asyncio workers — 4 concurrent scans = RAM spike that freezes Windows. Run lint checks synchronously (not in background); if a background lint job is pending, use Monitor to wait for it — never launch a second check for the same command. | System froze twice from concurrent black processes — Sprint 31 2026-07-28 |
| Pre-push hook scope: always run `ruff check .` from inside `backend/` (covers `scripts/`, `evals/`, all subdirs). Running only on `app/` + `tests/` misses errors in `scripts/`. Hook runs on full `backend/` path. | Two failed pushes because I ran `ruff check app/ tests/` (0 errors) but hook runs `ruff check backend/` (found errors in scripts/) — Sprint 31 2026-07-28 |
| On Windows, never use bash `nohup ... &` for background pipeline runs — the process dies when the bash shell session closes. Always use PowerShell `Start-Process -NoNewWindow -RedirectStandardOutput log -RedirectStandardError err_log`. | Loop 1/5 of 5-post generation sprint died after 4 log lines — bash nohup killed on shell exit — medium-agent-factory Sprint 40 2026-07-31 |
| At the start of EVERY turn where a background process was previously launched: immediately check its log before responding to anything else. Use `tail -5 <log>` + `ps`/`Get-Process` to determine: still running, completed, or crashed. If crashed: diagnose and restart. Do not wait for the user to notice. | User had to ask "a long time has passed, maybe the pipeline is ready" — loop had been dead for the entire conversation while the Monitor waited silently — medium-agent-factory Sprint 40 2026-07-31 |
| Slop detector regex `r"\?\*\*"` is too broad — it catches any bold closing question (`**text?**`) which the content generator is REQUIRED to write. Remove that pattern only; keep specific phrases (let me know, share in comments, drop a comment, what's the most). | READER_CTA_BAIT blocking quality_score=1.0 posts on `?**` — medium-agent-factory Sprint 41 2026-07-31 |
| When patching mock paths in pytest, patch where the symbol is USED not where it is DEFINED. `from module import func` creates a local binding; patching `module.func` changes the origin but not the local binding in the importing module. | `test_no_guide_subtype_uses_parallel_wave` failed because `_extract_grounding_ledger` was patched at `app.agents.content_generator.*` but used via `from ... import` in `app.agents.nodes.content_generation.*` — medium-agent-factory Sprint 41 2026-07-31 |
| Agent prompts in parallel-executor must reference plan file path (<=300 tokens), not paste code blocks. Pasting 200-line code blocks = ~3,000 tokens per agent; a 3-agent wave wastes ~9,000 tokens on context the agent reads from disk anyway. Pattern: `"Read Task N from docs/superpowers/plans/PLAN.md. Implement it."` | Wave 2 of job-matcher sprint 2026-08-11: prompts pasted full task code instead of referencing the plan file already on disk — system-curator identified the gap same session |
| Never use `claude-in-chrome` MCP for frontend E2E testing. Use Playwright MCP (`@playwright/mcp`) exclusively. Write `.spec.ts` files; use MCP session screenshots/assertions as test expectations. `claude-in-chrome` caused hung SSE readers, screenshot timeouts, and no written test artifacts. | Browser hung mid-SSE on job-matcher frontend sprint 2026-08-11; user stopped session and explicitly corrected. |
